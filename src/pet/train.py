def main():
    ...
    stop_decay_after = settings.pop("stop_decay_after", max_epochs)
    ...
    debug_nans = settings.pop("debug_nans", False)
    ...
    if compilation_cache:
        ...
        comms.talk(f"caching compilations to {cache_dir.resolve()}", full=True)
    ...
    def get_valid_iterator():
        return iter(
            DataLoader(
                ...
                sampler=IndexSampler(n_valid, num_epochs=1, seed=seed, shuffle=False),
                operations=[
                    *filters,
                    to_sample,
                    FilterEmpty(),
    ...
    def get_training_iterator(num_epochs):
        if use_rotational_augmentation:
            operations = [RandomRotation(), *filters, to_sample, FilterEmpty(), get_batcher()]
        else:
            operations = [*filters, to_sample, FilterEmpty(), get_batcher()]
    ...
    if benchmark_pipeline:
        ...
        reporter.step(f"benchmark pipeline ({probe_epochs} epochs)", spin=False)
        ...
        msg.append(f"              {worker_count} train workers, buffer {worker_buffer_size}")
        msg.append(
    ...
    if decay_style == "linear":
        ...
        lr_decay = {"style": "warmup_cosine", "warmup_epochs": warmup_epochs}
    else:
        raise ValueError
    ...
    class Manager:
        ...
        def report(
            ...
            msg += format_metrics(metrics["valid"], keys=keys, properties=properties)
            msg.append("")
    ...
    def predict_and_collate(params, batches):
        ...
        return (
            {key: np.concatenate(labels[key]) for key in keys},
            {key: np.concatenate(predictions[key]) for key in keys},
        )
